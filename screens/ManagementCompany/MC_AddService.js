...
import {createServices} from '../../src/graphql/mutations';
...
export default function MC_ActiveServices ({route,navigation}){
  ...
  useEffect(() => { 
    ...
    if(isupdtd){
      console.log('id ',id);
      setIsupdtd(false);
      navigation.navigate('MC_Servicemen',{param:id});
    }
    setDay('Select day');
    setSchedule('select ');
    setCategory('select ');
  },[etime,isupdtd,isFocused]);
  ...
  const onChange2 = (event, value) => {
    ...
    const handleSubmit = async () => {
      try {
        if(category.trim() == "select" || schedule.trim() == "select"|| day.trim() == "Select day")
                {
            ...
            const response=await API.graphql(graphqlOperation(createServices, {
              input: {
                category:category,
                schedule:schedule,
                day:day,
            ...
            await (setId(response.data.createServices.id));
            await(console.log('response id : ',id));
            setIsupdtd(true);
    ...
    return (
        ...
        <View style={styles.container}>
           <StatusBar animated = {true}
              ...
              <View >
                  <Text style={styles.Txt1}>Add New Service</Text>               
              </View>

              <View style = {styles.three}></View>

              <View style={styles.line1}>
              <Text style={styles.Txt2}>Select the Category</Text>
              </View>
              <SearchableDropdown
               // onTextChange={(text) => console.log(text)}
                //On text change listner on the searchable input
                onItemSelect={(item) =>setCategory(item.name) }
