...
export default function App ({route,navigation}){
  ...
  const getImage1 = async (imgs) => {
    try {
      for ( let i of imgs){
        const imageURL = await Storage.get(i);
       const ImageSrc = {uri: imageURL};
        setStphoto(current => [...current, ImageSrc]);   
    } }
    catch (e) {
      console.log(e);
    }
  };
  const getImage2 = async (imgs) => {
    try {
      for ( let i of imgs){
        const imageURL = await Storage.get(i);
       const ImageSrc = {uri: imageURL};
       setEdphoto(current => [...current, ImageSrc]);   
    } }
    catch (e) {
      console.log(e);
    }
    ...
    const fetchData =async() =>{
      try{
          ...
          const oneTodo = await API.graphql({ query: queries.getServiceDetails, variables: { id: p1 }});
          setData(oneTodo.data.getServiceDetails);
          await getImage1(oneTodo.data.getServiceDetails.startimgs);
          await getImage2(oneTodo.data.getServiceDetails.endimgs);
          setPos(true);
           ...
           <StatusBar animated = {true}
            ...
            <View style={styles.B1}>
                <View style={styles.V1}>
                    ...
                    <View style={{paddingRight:'25%'}}>
                      <Text style={styles.tt2}>{data.block}</Text>
                    </View>
              ...
              <View style={styles.t1}>
                  ...
                  <Text style={styles.text2}>{data.starttime.slice(17,27)}</Text>
              </View>
              ...
              {stphoto.map((element, index) => {
                    return (
                      <View key={index} style={styles.Box2}>
                         <Image style={{ width: '100%', height: '100%',resizeMode:'center'}} source={element} />
                      </View>
                    );
                  })}
              </ScrollView>:
               <View  style={styles.Box2}>
                    <Entypo name="camera" size={45} color="#888888" style={styles.camIcon}/>
                </View>
              }
            ...
            <View >
              ...
              <View style={styles.t1}>
                  ...
                  <Text style={styles.text2}>{data.endtime.slice(17,27)}</Text>
              </View>
            ...
            {/* <View style = {styles.Boxes}>
                <View style = {styles.filterB}>
                      <Text style={styles.filtxt}>Share</Text>
                     <Entypo name='share' size={25} style={styles.shareIcon}/>   
                </View>

                <View style = {styles.sortB}>
                    <Text style={styles.filtxt}>Download</Text>
                   
                     <Entypo name="download" size={25} style={styles.downIcon}/>
