...
import {createServiceDetails,updateServiceDetails} from '../../src/graphql/mutations';
...
export default function App ({route,navigation}){
    ...
    const [efinal, setEfinal] = useState([]);
    const [edphoto, setEdphoto] = useState([]);
  ...
  const openCamera1 = async () => {
    ...
    if (!result.cancelled) {
      setStphoto(current => [...current, result.uri]);
      const newImageUri =  "file:///" + result.uri.split("file:/").join("");
        ...
        setSfinal(current => [...current, newImageUri.split("/").pop()]);
  ...
  const openCamera2 = async () => {
    ...
    if (!result.cancelled) {
      setEdphoto(current => [...current, result.uri]);
      const newImageUri =  "file:///" + e.split("file:/").join("");
      const photoResponse = await fetch(newImageUri);
      const blob = await photoResponse.blob();
      await Storage.put(newImageUri.split("/").pop(), blob, {
        contentType: mime.getType(newImageUri),
      });
      setEfinal(current => [...current, newImageUri.split("/").pop()]);
     // console.log(newImageUri.split("/").pop());
    }
  };
  ...
  const handleSubmit1 = async () =>{
    try {
    console.log('started');
        const response=await API.graphql(graphqlOperation(createServiceDetails, {
          input: {
            startimgs:sfinal,
  ...
  const handleSubmit2 = async () =>{
    try {
        const response=await API.graphql(graphqlOperation(updateServiceDetails, {
          input: {
            id:data1.id,
            endimgs:efinal,
    ...
    const fetchData =async() =>{
      try{
            ...
            const oneTodo = await API.graphql({ query: queries.getServices, variables: { id: p1 }});
          setData(oneTodo.data.getServices);
          console.log('data:: ',oneTodo);
          
          let filter = {
              and: [{ servicesid: {eq:p1} },
                { servicemanid: {eq:user.attributes['email']} },
                { endtime: {eq:'na'} }]

        };
        const unfinished = await API.graphql({ query: queries.listServiceDetails, variables: { filter: filter}});
        setData1(unfinished.data.listServiceDetails.items[0]);
        console.log('unfinished job details-->',data1);
          ...
          <View style={styles.B1}>
            ...
            {edphoto.map((element, index) => {
              ...
              <View  style={styles.Box2}>
                  <AntDesign name="upload" size={35} color="#888888"  onPress={openCamera2} style={styles.camIcon} />
              </View>
            }
           ...
           <StatusBar animated = {true}
            ...
            <View style={styles.B1}>
              ...
              {stphoto.map((element, index) => {
               ...
               <View  style={styles.Box2}>
                    <AntDesign name="upload" size={35} onPress={openCamera2} color="#888888" style={styles.camIcon} />
                </View>
              }
